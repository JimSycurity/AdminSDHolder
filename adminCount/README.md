...
After some fiddling around it looks like I'll either need to resort to using DCShadow or likely offline editing of the NTDS.dit file using an ESE editor. DSInternals looked promising at first, but it does not appear to have functionality to delete objects of any class. The offline access, editing, and removal commands are targetted at security principals and groups. I'll (hopefully) come back to this at a later date when I have more time to spend on deleting an object that's not intended to be deleted.
